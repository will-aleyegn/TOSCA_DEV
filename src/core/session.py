...
class SessionManager:
    """
    Manages treatment sessions.

    Handles session creation, lifecycle, and persistence.
    """
    ...
    def create_session(
        ...
        Args:
            ...
            operator_id: Operator/technician identifier
    ...
    def end_session(self, abort: bool = False, reason: str = "") -> None:
        """
        End current session.

        Args:
            abort: If True, mark as aborted instead of completed
            reason: Reason for abort (if applicable)
        """
        ...
        if abort:
            self.current_session.abort(reason)
        else:
