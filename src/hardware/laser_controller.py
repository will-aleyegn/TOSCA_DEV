...
- Power control (current and power modes)
- Temperature monitoring (TEC)
...
class LaserController(QObject):
    ...
    temperature_changed = pyqtSignal(float)  # TEC temperature in °C
    ...
    def _read_limits(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Limits: Current={self.max_current_ma:.0f}mA, "
                f"Temp={self.min_temperature_c:.1f}-{self.max_temperature_c:.1f}°C"
            )
